...
SENSOR_ACCURACY = 0.5
SIMULATION_START = "2024-10-16 00:00:00"
SIMULATION_END = "2025-06-05 00:00:00"
...
OUTPUT_DIR = r"E:\Research_Assistant\Tarun\Analysis_Results\Tarun\Analysis_Results\Staged_workflow\RC_baeline_timeconstant\iter2"
...
if __name__ == "__main__":
    ...
    for key, exp_df in exp_dict.items():
        ...
        exp = exp_df.iloc[:, 0].copy()  # Use first column
        ...
        plt.title(
            f"{bay} - {key} Zone\n"
            f"MAE: {mae:.2f} °F | RMSE: {rmse:.2f} °F\n"
            f"MAE-Day: {mae_day:.2f} °F | MAE-Night: {mae_night:.2f} °F\n"
            f"RMSE-Day: {rmse_day:.2f} °F | RMSE-Night: {rmse_night:.2f} °F"
        )
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
    ...
    try:
        roof_exp = exp_dict["Roof"].iloc[:, 0].resample('1h').mean()
        ceil_exp = exp_dict["Ceiling"].iloc[:, 0].resample('1h').mean()
