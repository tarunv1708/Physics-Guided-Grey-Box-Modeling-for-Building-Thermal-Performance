    ...
    def __getitem__(self, idx):
        ...
        for i in range(min(len(seq_data), len(seq_weather))):
            try:
                ...
                # ✅ Correct — Add T_zone in the input vector
                input_vector = np.concatenate([time_feat, weather, [hvac, tzone]])
...
class ThermalPINN(nn.Module):
    ...
    def forward(self, features, baseline):
        ...
        ghi = features[:, :, 4]  # GHI index assumed correct
...
def train_pinn(model, dataloader, epochs=1000):
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            ghi = features[:, :, 3]  # assuming GHI is 4th feature
            is_night = (ghi < 10).float()
...
# ===================== CONFIGURATION =====================
SIMULATION_START = "2024-12-16"
SIMULATION_END = "2025-02-27"
...
def stratified_temporal_split_by_months(sequences, train_months=[ 10,11,12], test_months=[1,2]):
...
def load_simulation_data():
    sim = pd.read_csv(
        r"E:\Research_Assistant\Tarun\Analysis_Results\Tarun\Analysis_Results\Staged_workflow\Cold_month_PSO\iter2\South_Bay\PINN_input_20250625_043513.csv",
    ...
    return sim.loc["2024-12-16":"2025-02-27"]
...
if __name__ == "__main__":
    ...
    OUTPUT_DIR = os.path.join(r"E:\Research_Assistant\Tarun\Analysis_Results\Tarun\Analysis_Results\Staged_workflow\Cold_month_PINN\iter2", bay, datetime.now().strftime("%Y%m%d_%H%M%S"))
    ...
    train_seqs, test_seqs = stratified_temporal_split_by_months(
        valid_sequences,
        train_months=[  10,11,12],  # October and November for training
        test_months=[1,2]  # December for testing
    )
    ...
    # 🔍 Plot delta_pred vs delta_true (Roof only) with explicit sequence labels
